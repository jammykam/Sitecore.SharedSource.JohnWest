namespace Sitecore.Sharedsource.Shell.Applications.ContentEditor
{
  ...
  public class TreeList : SC.Shell.Applications.ContentEditor.TreeList
  {
    ...
    // the item selected in the Content Editor
    private SC.Data.Items.Item currentItem;
    ...
    // the ID of the item selected in the Content Editor
    // (Sitecore sets this property automatically if it exists)
    // overridden to store in view state
    ...
    // the version of the item selected in the Content Editor
    // (Sitecore sets this property automatically if it exists)
    ...
    protected SC.Data.Items.Item CurrentItem
    {
      get
      {
        ...
        {
          this.currentItem = SC.Context.ContentDatabase.GetItem(
            this.ItemID,
            Sitecore.Globalization.Language.Parse(this.ItemLanguage),
            SC.Data.Version.Parse(this.ItemVersion));
          SC.Diagnostics.Assert.IsNotNull(this.currentItem, "item");
        }
    ...
    protected string ExpandDataSource(string dataSource)
    {
      ...
      return queried[0].Paths.FullPath;
    }
